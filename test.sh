...
./build_"$FRAMEWORK"/test_"$FRAMEWORK" -m=compare -e=3 $PATHES -if=*.ppm -rn=$NUMBER -wt=1 -tt=$THREAD -tf=$FORMAT -bs=$BATCH -t=0.001 -dp=0 -dpf=6 -dpl=2 -ar=0 -rt=0.3 -ln=$LOG
if [ $? -ne 0 ]
then
  echo "Test $DIR is failed!"
  exit
...
TEST inference_engine test_008 5 1 0 1 001a
